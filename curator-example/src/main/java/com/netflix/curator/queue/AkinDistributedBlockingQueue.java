package com.netflix.curator.queue;
...
/**
 * <p> An implementation of the Distributed Queue ZK recipe which implement BlockingQueue interface and
 * QueueConsumer interface. Items put into the queue are guaranteed to be ordered (by means of ZK's PERSISTENT_SEQUENTIAL node) and some items are consumed by this queue</p>
...
public class AkinDistributedBlockingQueue<E> implements BlockingQueue<E>, QueueConsumer<E> {
    ...
    @Override
    public void consumeMessage(E e) throws Exception {
        localInternalQueue.add(e);
